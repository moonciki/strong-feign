...
public final class RequestTemplate implements Serializable {
	 ...
	 *            for the request.
	 * @param body
	 *            of the request, may be null
	 * @param decodeSlash
	 *            if the request uri should encode slash characters.
	 * @param collectionFormat
	 *            when expanding collection based variables.
	 */
	private RequestTemplate(String target, String fragment, UriTemplate uriTemplate, HttpMethod method, Charset charset,
			Request.Body body, boolean decodeSlash, CollectionFormat collectionFormat) {
		this.target = target;
	...
	public static RequestTemplate from(RequestTemplate requestTemplate) {
		RequestTemplate template = new RequestTemplate(requestTemplate.target, requestTemplate.fragment,
				requestTemplate.uriTemplate, requestTemplate.method, requestTemplate.charset, requestTemplate.body,
				requestTemplate.decodeSlash, requestTemplate.collectionFormat);
	 ...
	 * Variable values will be pct-encoded, if they are not already.
	 *
	 * @param variables
	 *            containing the variable values to use when resolving expressions.
	...
	public RequestTemplate resolve(Map<String, ?> variables) {
		...
		/*
		 * for simplicity, combine the queries into the uri and use the resulting uri to
		 * seed the resolved template.
		 */
		if (!this.queries.isEmpty()) {
			/*
			 * since we only want to keep resolved query values, reset any queries on the
			 * resolved copy
			 */
		...
		if (!this.headers.isEmpty()) {
			/*
			 * same as the query string, we only want to keep resolved values, so clear the
			 * header map on the resolved instance
			 */
	...
	/**
	 * Resolves all expressions, using the variables provided. Values not present in
	 * the {@code
	 ...
	 * @deprecated use {@link RequestTemplate#resolve(Map)}. Values already encoded
	 *             are recognized as such and skipped.
	 */
	...
	/**
	 * Creates a {@link Request} from this template. The template must be resolved
	 * before calling this method, or an {@link IllegalStateException} will be
	 * thrown.
	 *
	 * @return a new Request instance.
	 * @throws IllegalStateException
	 *             if this template has not been resolved.
	 */
	...
	/**
	 * Set whether do encode slash {@literal /} characters when resolving this
	 * template.
	 *
	 * @param decodeSlash
	 *            if slash literals should not be encoded.
	 * @return a RequestTemplate for chaining.
	 */
	 ...
	 * {@link Iterable}s or {@link Collection}s
	 *
	 * @param collectionFormat
	 *            to use.
	 * @return a RequestTemplate for chaining.
	...
	/**
	 * The Collection Format that will be used when resolving {@link Iterable} and
	 * {@link Collection} variables.
	 *
	 ...
	 * <p>
	 * This method is poorly named and is used primarily to store the relative uri
	 * for the request. It has been replaced by {@link RequestTemplate#uri(String)}
	 ...
	 * <p>
	 * This method is poorly named has undocumented behavior. When the value
	 * contains a fully qualified http request url, the value is always inserted at
	 * the beginning of the uri.
	 * </p>
	 * <p>
	 * Due to this, use of this method is not recommended and remains for backward
	 * compatibility. It has been replaced by {@link RequestTemplate#target(String)}
	 * and will be removed in a future release.
	 * </p>
	 *
	 * @param pos
	 *            in the uri to place the value.
	 * @param value
	 *            to insert.
	 * @return a RequestTemplate for chaining.
	 ...
	 *            to use, must be a relative uri.
	 * @param append
	 *            if the uri should be appended, if the uri is already set.
	 * @return a RequestTemplate for chaining.
	 */
	public RequestTemplate uri(String uri, boolean append) {
		...
		/*
		 * templates may provide query parameters. since we want to manage those
		 * explicity, we will need to extract those out, leaving the uriTemplate with
		 * only the path to deal with.
		 */
		Matcher queryMatcher = QUERY_STRING_PATTERN.matcher(uri);
		if (queryMatcher.find()) {
	 ...
	 *
	 * @param target
	 *            host for this request. Must be an absolute target.
	 * @return a RequestTemplate for chaining.
	 */
	public RequestTemplate target(String target) {
		...
		try {
			...
			if (Util.isNotBlank(targetUri.getRawQuery())) {
				/*
				 * target has a query string, we need to make sure that they are recorded as
				 * queries
				 */
	...
	/**
	 * The URL for the request. If the template has not been resolved, the url will
	 * represent a uri template.
	 *
	 * @return the url
	 */
	public String url() {
	...
	/**
	 * Specify a Query String parameter, with the specified values. Values can be
	 * literals or template expressions.
	 *
	 * @param name
	 *            of the parameter.
	 * @param values
	 ...
	 *            for this parameter.
	 * @param collectionFormat
	 *            to use when resolving collection based expressions.
	 ...
	 *            of the parameter.
	 * @param values
	 *            for the parameter, may be expressions.
	 * @param collectionFormat
	 *            to use when resolving collection based query variables.
	 * @return a RequestTemplate for chaining.
	 */
	...
	/**
	 * Specify a Header, with the specified values. Values can be literals or
	 ...
	 *            of the header
	 * @param values
	 *            for the header, may be expressions.
	 * @return a RequestTemplate for chaining.
	 */
	...
	/**
	 * Set the Body for this request. Charset is assumed to be UTF_8. Data must be
	 * encoded.
	 *
	 * @param bodyText
	 *            to send.
	...
	/**
	 * Return if the variable exists on the uri, query, or headers, in this
	 * template.
	 *
	 * @param variable
	 *            to look for.
	...
	private void extractQueryTemplates(String queryString, boolean append) {
		...
		Map<String, List<String>> queryParameters = Arrays.stream(queryString.split("&")).map(this::splitQueryParameter)
