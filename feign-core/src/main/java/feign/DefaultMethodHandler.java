...
/**
 * Handles default methods by directly invoking the default method code on the
 * interface. The bindTo method must be called on the result before invoke is
 * called.
 */
@IgnoreJRERequirement
final class DefaultMethodHandler implements MethodHandler {
	// Uses Java 7 MethodHandle based reflection. As default methods will only exist
	// when
	...
	/**
	 * Bind this handler to a proxy object. After bound, DefaultMethodHandler#invoke
	 * will act as if it was called on the proxy object. Must be called once and
	 * only once for a given instance of DefaultMethodHandler
	 */
