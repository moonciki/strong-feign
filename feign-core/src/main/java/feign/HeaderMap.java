 ...
 * over any previously specified header parameters. <br>
 * This parameter is useful in cases where different header fields and values
 * need to be set on an API method on a per-request basis in a thread-safe
 * manner and independently of Feign client construction. A concrete example of
 * a case like this are custom metadata header fields (e.g. as "x-amz-meta-*" or
 * "x-goog-meta-*") where the header field names are dynamic and the range of
 * keys cannot be determined a priori. The {@link Headers} annotation does not
 * allow this because the header fields that it defines are static (it is not
 * possible to add or remove fields on a per-request basis), and doing this
 * using a custom {@link Target} or {@link RequestInterceptor} can be cumbersome
 * (it requires more code for per-method customization, it is difficult to
 * implement in a thread-safe manner and it requires customization when the
 * Feign client for the API is built). <br>
 * 
 * <pre>
 * ...
 ...
 * method, except in the following cases: <br>
 * <br>
 * <ul>
