 ...
 * Converting out to a throttle exception are examples of this in use.
 *
 * <p/>
 * Ex:
 *
 ...
 * <p/>
 * It is commonly the case that 404 (Not Found) status has semantic value in
 * HTTP apis. While the default behavior is to raise exeception, users can
 * alternatively enable 404 processing via
...
public interface ErrorDecoder {
	 ...
	 * application-specific exceptions where possible. If your exception is
	 * retryable, wrap or subclass {@link RetryableException}
	 *
	 * @param methodKey
	 *            {@link feign.Feign#configKey} of the java method that invoked the
	 *            request. ex. {@code IAM#getUser()}
	 * @param response
	 *            HTTP response where {@link Response#status() status} is greater
	 *            than or equal to {@code 300}.
	 ...
	 *         implementation. If the throwable is retryable, it should be wrapped,
	 *         or a subtype of {@link RetryableException}
	 */
	public Exception decode(String methodKey, Response response);
	...
	/**
	 * Decodes a {@link feign.Util#RETRY_AFTER} header into an absolute date, if
	 * possible. <br>
	...
	static class RetryAfterDecoder {
		 ...
		 * @param retryAfter
		 *            String in
		 *            <a href="https://tools.ietf.org/html/rfc2616#section-14.37"
