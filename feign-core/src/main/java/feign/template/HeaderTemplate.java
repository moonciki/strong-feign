...
/**
 * Template for HTTP Headers. Variables that are unresolved are ignored and
...
public final class HeaderTemplate extends Template {
	...
	public static HeaderTemplate append(HeaderTemplate headerTemplate, Iterable<String> values) {
		...
		headerValues.addAll(
				StreamSupport.stream(values.spliterator(), false).filter(Util::isNotBlank).collect(Collectors.toSet()));
