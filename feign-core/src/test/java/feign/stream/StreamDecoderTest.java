...
public class StreamDecoderTest {
	...
	public void simpleStreamTest() {
		...
		StreamInterface api = Feign.builder()
				.decoder(StreamDecoder
						.create((response, type) -> new BufferedReader(response.body().asReader()).lines().iterator()))
				.doNotCloseAfterDecode().target(StreamInterface.class, server.url("/").toString());
	...
	public void shouldCloseIteratorWhenStreamClosed() throws IOException {
		Response response = Response.builder().status(200).reason("OK").headers(Collections.emptyMap())
