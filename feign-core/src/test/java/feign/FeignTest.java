...
public class FeignTest {
	...
	public void configKeyFormatsAsExpected() throws Exception {
		...
		assertEquals("TestInterface#uriParam(String,URI,String)", Feign.configKey(TestInterface.class,
	...
	/**
	 * when you must parse a 2xx status to determine if the operation succeeded or
	 * not.
	 */
	@Test
	...
	public void throwsOriginalExceptionAfterFailedRetries() throws Exception {
		...
		TestInterface api = Feign.builder().exceptionPropagationPolicy(UNWRAP).retryer(new Retryer.Default(1, 1, 2))
	...
	public void whenReturnTypeIsResponseNoErrorHandling() {
		...
		TestInterface api = Feign.builder().client((request, options) -> response).target(TestInterface.class,
	...
	public void decodingDoesNotSwallow404ErrorsInDecode404Mode() throws Exception {
		...
		TestInterface api = new TestInterfaceBuilder().decode404().errorDecoder(new IllegalArgumentExceptionOn404())
				.target("http://localhost:" + server.getPort());
	...
	public void equalsHashCodeAndToStringWork() {
		...
		Target<OtherTestInterface> t3 = new HardCodedTarget<OtherTestInterface>(OtherTestInterface.class,
				"http://localhost:8080");
	...
	public void mapAndDecodeExecutesMapFunction() throws Exception {
		...
		TestInterface api = new Feign.Builder().mapAndDecode(upperCaseResponseMapper(), new StringDecoder())
				.target(TestInterface.class, "http://localhost:" + server.getPort());
