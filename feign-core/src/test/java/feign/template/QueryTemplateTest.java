...
public class QueryTemplateTest {
	...
	public void templateToQueryString() {
		QueryTemplate template = QueryTemplate.create("name", Arrays.asList("Bob", "James", "Jason"), Util.UTF_8);
		assertThat(template.toString()).isEqualToIgnoringCase("name=Bob&name=James&name=Jason");
	}

	@Test
	public void expandSingleValue() {
		QueryTemplate template = QueryTemplate.create("name", Collections.singletonList("{value}"), Util.UTF_8);
	...
	public void collectionFormat() {
		QueryTemplate template = QueryTemplate.create("name", Arrays.asList("James", "Jason"), Util.UTF_8,
	...
	public void expandPureParameterWithSlash() {
		QueryTemplate template = QueryTemplate.create("/path/{name}", Collections.emptyList(), Util.UTF_8);
		String expanded = template.expand(Collections.singletonMap("name", "firsts"));
	...
	public void expandNameUnresolved() {
		QueryTemplate template = QueryTemplate.create("{parameter}", Arrays.asList("James", "Jason"), Util.UTF_8);
		String expanded = template.expand(Collections.singletonMap("name", "firsts"));
	...
	public void expandSingleValueWithComma() {
		QueryTemplate template = QueryTemplate.create("collection", Collections.singletonList("{collection}"),
				Util.UTF_8);
	...
	public void expandSingleValueWithJson() {
		QueryTemplate template = QueryTemplate.create("json", Collections.singletonList("{json}"), Util.UTF_8);
		String expanded = template.expand(Collections.singletonMap("json", "{\"name\":\"feign\",\"version\": \"10\"}"));
		assertThat(expanded).isEqualToIgnoringCase("json=%7B%22name%22:%22feign%22,%22version%22:%20%2210%22%7D");
	}
