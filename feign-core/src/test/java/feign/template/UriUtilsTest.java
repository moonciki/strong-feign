...
public class UriUtilsTest {
	...
	/**
	 * Verify that values outside of the allowed characters in a path segment are
	 * pct-encoded. The list of approved characters used are those listed in <a
	 * href="https://tools.ietf.org/html/rfc3986#appendix-A>RFC 3986 Appendix A</a>
	 */
	@Test
	public void pctEncodeReservedPathCharacters() {
		/*
		 * the only not allowed characters are the general delimiters, with the
		 * exception of slash, question, colon and at
		 */
		String reservedPath = "/api/user@host:port#section[a-z]/data";
		String reservedPathEncoded = UriUtils.pathEncode(reservedPath, UTF_8);

		/*
		 * the result should be the path, with the slash and question retained, but all
		 * other special characters encoded
		 */
	...
	/**
	 * Verify that the list of allowed characters in a path segment are not
	 * pct-encoded, as they don't have to be. The list of approved characters used
	 * are those listed in <a
	 * href="https://tools.ietf.org/html/rfc3986#appendix-A>RFC 3986 Appendix A</a>
	 */
	@Test
	...
	/**
	 * Verify that when a full query string is provided, only unapproved characters
	 * are pct-encoded. This differs from
	 * {@link UriUtils#queryParamEncode(String, Charset)} in that this method
	...
	/**
	 * Verify that a value meant for a query string parameter is pct-encoded using
	 * the defined query set of allowed characters, including equals, ampersands and
	 * question marks as in Feign, we manage the creation of the key value pair.
	 *
	 * The list of approved characters used are those listed in <a
	 * href="https://tools.ietf.org/html/rfc3986#appendix-A>RFC 3986 Appendix A</a>
	 */
