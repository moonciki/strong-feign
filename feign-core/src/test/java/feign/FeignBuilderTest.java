...
public class FeignBuilderTest {
	...
	public void testNoFollowRedirect() {
		...
		String url = "http://localhost:" + server.getPort();
		TestInterface noFollowApi = Feign.builder().options(new Request.Options(100, 600, false))
				.target(TestInterface.class, url);

		Response response = noFollowApi.defaultMethodPassthrough();
		assertThat(response.status()).isEqualTo(302);
		assertThat(response.headers().getOrDefault("Location", null)).isNotNull()
				.isEqualTo(Collections.singletonList("/"));

		server.enqueue(new MockResponse().setResponseCode(302).addHeader("Location", "/"));
		server.enqueue(new MockResponse().setResponseCode(200));
		TestInterface defaultApi = Feign.builder().options(new Request.Options(100, 600, true))
				.target(TestInterface.class, url);
	 ...
	 *
	 * It does so by creating a custom Decoder that lazily retrieves the response
	 * body when asked for it and pops the value into an Iterator.
	 *
	 * Without the doNoCloseAfterDecode flag, the test will fail with a "stream is
	 * closed" exception.
	 *
	 * @throws Exception
	 */
	...
	public void testDoNotCloseAfterDecode() throws Exception {
		...
		TestInterface api = Feign.builder().decoder(decoder).doNotCloseAfterDecode().target(TestInterface.class, url);
	...
	public void testDoNotCloseAfterDecodeDecoderFailure() throws Exception {
		...
		TestInterface api = Feign.builder().client(new Client() {
			Client client = new Client.Default(null, null);

			@Override
			public Response execute(Request request, Request.Options options) throws IOException {
				final Response original = client.execute(request, options);
				return Response.builder().status(original.status()).headers(original.headers())
						.reason(original.reason()).request(original.request()).body(new Response.Body() {
							@Override
